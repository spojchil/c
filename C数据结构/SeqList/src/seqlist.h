...
// 线性表
#define SEQ_LIST_DEFAULT_CAP 10
typedef struct{
...
// 设置元素
bool seq_list_set(SeqList* plist, SeqListElem* elem, int index);

// 查找元素
int seq_list_find(SeqList* plist, SeqListElem* elem);

// 读取元素，不使用返回指针的写法，因为那个写法不安全也破坏了封装性
bool seq_list_get(const SeqList* plist, int index, SeqListElem* value);

// 删除元素,根据索引
bool seq_list_remove(SeqList* plist, int index);
